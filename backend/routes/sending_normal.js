...
router.get("/check-user", (req, res) => {
    ...
    db.query(query, [username], (err, results) => {
      ...
      return res.status(200).json({
        exists: true,
        message: "✅ User exists",
        name: user.name, // only sending name
        // or you could spread full details like: ...user
      });
    });
  });
  ...
  router.post("/transfer", async (req, res) => {
    ...
    // 1. Verify sender password
    db.query("SELECT * FROM users WHERE user_name = ?", [sender], async (err, results) => {
      if (err || results.length === 0) return res.status(400).json({ message: "Sender not found." });
        ...
        db.query(updateSender, [newSenderBalance, sender_username], err => {
          ...
          db.query(updateReceiver, [amount, receiver_username], err => {
            ...
            db.query(insertPayment, [sender_username, receiver_username, amount, type], err => {
              ...
              // Optional: If type is 'extra', insert into extra_bal
              if (type === "extra") {
                const insertExtra = `
